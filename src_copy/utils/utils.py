...
def visualize_attack_result(clean_img, adv_img, save_name="attack_result.png"):
    ...
    axes[1].set_title("Perturbation (*50)", fontsize=14)
    axes[1].axis('off')
    ...
    plt.savefig(pdf_name, dpi=300, bbox_inches='tight', pad_inches=0.1)

    print(f"[Utils] Visualization saved to {save_name} and {pdf_name}")
    plt.close()
...
def plot_performance_comparison(final_results, save_name="experiment_summary.png"):
    ...
    # 使用更学术的配色，例如深蓝和深红
    rects1 = ax.bar(x - width / 2, r1_attack, width, label='No Defense', color='#1f77b4', alpha=0.9)
    rects2 = ax.bar(x + width / 2, r1_defense, width, label='With JPEG Defense', color='#d62728', alpha=0.9)

    ax.set_ylabel('Recall@1 (%)', fontsize=12)
    ax.set_title('Attack Impact & Defense Effectiveness', fontsize=14)
    ax.set_xticks(x)
    ...
    print(f"[Utils] Performance chart saved to {save_name} and {pdf_name}")
    plt.close()