...
def calculate_recall_metrics(image_embeds, text_embeds, ground_truth_indices, k_list=[1, 3, 5, 10]):
    """
...
def plot_performance_comparison(final_results, save_name="experiment_summary.png"):
    """
    根据实验结果列表绘制分组柱状图：展示攻击效果及防御效果。
    Args:
        final_results: list of dict, 包含 'Method', 'R@1' 等键
        save_name: 保存文件名
    """
    # 1. 数据解析
    # 我们需要分离出基础方法（如 FGSM）和防御后方法（如 FGSM + JPEG）
    methods = []
    r1_attack = []
    r1_defense = []
    ...
    # 遍历列表，只找基础方法（不含 "+" 的，或者 Clean）
    for item in final_results:
        name = item['Method']
        if "+" not in name:  # 这是一个基础方法 (Clean, FGSM, PGD...)
            methods.append(name)
            r1_attack.append(item['R@1'])

            # 尝试找对应的防御结果
            def_name = name + " + JPEG"
            if def_name in res_dict:
                r1_defense.append(res_dict[def_name])
            else:
                # 如果没有防御结果（比如 Clean），就用原值占位，或者设为 0
                # 这里为了图表美观，Clean 的防御柱子设为和原值一样（表示防御对 Clean 影响不大）
                r1_defense.append(item['R@1'])
    ...
    # 画两组柱子
    rects1 = ax.bar(x - width / 2, r1_attack, width, label='No Defense', color='#4e79a7')
    rects2 = ax.bar(x + width / 2, r1_defense, width, label='With JPEG Defense', color='#f28e2b')

    # 3. 设置图表属性
    ax.set_ylabel('Recall@1 (%)')
    ax.set_title('Attack Impact & Defense Effectiveness (Lower R@1 = Better Attack)')
    ax.set_xticks(x)
